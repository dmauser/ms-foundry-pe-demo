...
validate_s2() {
    ...
    if [[ -z "$rg" ]] || ! rg_exists "$rg"; then
        add_result S2 "Deployment present" SKIP "no .deploy-suffix / resource group — not deployed"
        return
    fi
    ...
    if [[ -z "$subnet" ]]; then
        ...
        if [[ -n "$exists" ]]; then add_result S2 "NSP app has NO VNet integration" PASS "identity-only path (correct)"
        else add_result S2 "NSP app has NO VNet integration" SKIP "$nspApp not found"; fi
    else add_result S2 "NSP app has NO VNet integration" FAIL "unexpectedly integrated: $subnet"; fi
    ...
    if [[ -n "$lawId" ]]; then
        ...
        if [[ -n "$nspId" ]]; then
            ...
            else add_result S2 "Diagnostic setting nsp-access-logs" WARN "not found on NSP"; fi
        fi
    else add_result S2 "Log Analytics workspace" SKIP "$law not found"; fi
}
...
validate_s3() {
    ...
    if [[ -z "$rg" ]] || ! rg_exists "$rg"; then
        add_result S3 "Deployment present" SKIP "no .deploy-suffix-agent / resource group — not deployed"
        return
    fi

    local stg; stg="$(az_q storage account list -g "$rg" --query "[].publicNetworkAccess" -o tsv)"
    if [[ -n "$stg" ]] && ! grep -q Enabled <<<"$stg"; then add_result S3 "Storage publicNetworkAccess" PASS "Disabled"
    elif [[ -n "$stg" ]]; then add_result S3 "Storage publicNetworkAccess" FAIL "$stg" 1
    else add_result S3 "Storage publicNetworkAccess" SKIP "no storage account found"; fi

    local cos; cos="$(az_q cosmosdb list -g "$rg" --query "[].publicNetworkAccess" -o tsv)"
    if [[ -n "$cos" ]] && ! grep -q Enabled <<<"$cos"; then add_result S3 "Cosmos DB publicNetworkAccess" PASS "Disabled"
    elif [[ -n "$cos" ]]; then add_result S3 "Cosmos DB publicNetworkAccess" FAIL "$cos" 1
    else add_result S3 "Cosmos DB publicNetworkAccess" SKIP "no Cosmos account found"; fi

    local srch; srch="$(az_q search service list -g "$rg" --query "[].publicNetworkAccess" -o tsv)"
    if grep -qi disabled <<<"$srch"; then add_result S3 "AI Search publicNetworkAccess" PASS "disabled"
    elif [[ -n "$srch" ]]; then add_result S3 "AI Search publicNetworkAccess" FAIL "$srch" 1
    else add_result S3 "AI Search publicNetworkAccess" SKIP "no Search service found"; fi

    local aipna; aipna="$(az_q cognitiveservices account list -g "$rg" --query "[].properties.publicNetworkAccess" -o tsv)"
    if [[ -n "$aipna" ]] && ! grep -q Enabled <<<"$aipna"; then add_result S3 "Foundry publicNetworkAccess" PASS "Disabled"
    elif [[ -n "$aipna" ]]; then add_result S3 "Foundry publicNetworkAccess" FAIL "$aipna" 1
    else add_result S3 "Foundry publicNetworkAccess" SKIP "no Foundry account found"; fi

    local pe; pe="$(az_q network private-endpoint list -g "$rg" --query "[].privateLinkServiceConnections[].privateLinkServiceConnectionState.status" -o tsv)"
    if [[ -n "$pe" ]]; then
        local bad; bad="$(grep -v Approved <<<"$pe" || true)"
        if [[ -z "$bad" ]]; then add_result S3 "Private endpoints approved" PASS "$(grep -c . <<<"$pe") endpoint(s) Approved"
        else add_result S3 "Private endpoints approved" FAIL "$(tr '\n' ',' <<<"$bad")" 1; fi
    else add_result S3 "Private endpoints approved" FAIL "no private endpoints found" 1; fi
    ...
    if [[ -z "$missing" && -n "$zones" ]]; then add_result S3 "Six private DNS zones present" PASS "6/6 core zones"
    elif [[ -n "$zones" ]]; then add_result S3 "Six private DNS zones present" FAIL "missing: $missing" 1
    else add_result S3 "Six private DNS zones present" SKIP "no private DNS zones found"; fi
    ...
    if [[ -n "$vnet" ]]; then
        ...
        if [[ "$linkedAll" == "1" ]]; then add_result S3 "DNS zones VNet-linked" PASS "linked to $vnet"
        else add_result S3 "DNS zones VNet-linked" FAIL "one or more zones not linked to the agent VNet" 1; fi

        local ad; ad="$(az_q network vnet subnet show -g "$rg" --vnet-name "$vnet" -n agent-subnet --query "delegations[].serviceName" -o tsv)"
        if grep -q "Microsoft.App/environments" <<<"$ad"; then add_result S3 "agent-subnet delegation" PASS "Microsoft.App/environments"
        elif [[ -n "$ad" ]]; then add_result S3 "agent-subnet delegation" WARN "$ad"
        else add_result S3 "agent-subnet delegation" SKIP "agent-subnet not found"; fi

        local pd; pd="$(az_q network vnet subnet show -g "$rg" --vnet-name "$vnet" -n app-subnet --query "delegations[].serviceName" -o tsv)"
        if grep -q "Microsoft.Web/serverFarms" <<<"$pd"; then add_result S3 "app-subnet delegation" PASS "Microsoft.Web/serverFarms"
        elif [[ -n "$pd" ]]; then add_result S3 "app-subnet delegation" WARN "$pd"
        else add_result S3 "app-subnet delegation" SKIP "app-subnet not found"; fi
    else add_result S3 "VNet present" FAIL "no VNet found in RG" 1; fi
    ...
    if [[ -n "$web" ]]; then
        ...
        if grep -q "/subnets/app-subnet" <<<"$subnet"; then add_result S3 "Test WebApp VNet-integrated" PASS "app-subnet"
        elif [[ -n "$subnet" ]]; then add_result S3 "Test WebApp VNet-integrated" WARN "${subnet##*/}"
        else add_result S3 "Test WebApp VNet-integrated" FAIL "not integrated" 1; fi
    else add_result S3 "Test WebApp VNet-integrated" SKIP "no WebApp found"; fi
}
