...
if [[ ! -f "$SUFFIX_FILE" ]]; then
    echo "❌ No .deploy-suffix-s3 file found. Run the Scenario 3 baseline (01 --scenario s3) and Step A (04) first."
    exit 1
fi
...
az deployment group create \
    ...
    --template-file "$REPO_ROOT/infra/05-nsp-enforce.bicep" \
...
echo ""
echo "════════════════════════════════════════════════════════════════"
echo "✓ Scenario 3 Step B complete! Foundry is now behind the NSP (Enforced)."
