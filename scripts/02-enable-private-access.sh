...
if [ ! -f "$SUFFIX_FILE" ]; then
    echo "ERROR: Suffix file not found at $SUFFIX_FILE"
    echo "Please run scripts/01-deploy-public-access.sh first."
    exit 1
fi
...
# Private Endpoint
PRIVATE_ENDPOINT_NAME="foundry-demo-pe-${SUFFIX}"
PRIVATE_ENDPOINT_CONNECTION="foundry-demo-connection-${SUFFIX}"

# Private DNS
PRIVATE_DNS_ZONE="privatelink.cognitiveservices.azure.com"
PRIVATE_DNS_LINK="foundry-demo-dns-link-${SUFFIX}"
...
echo "   curl \"https://${FOUNDRY_NAME}.cognitiveservices.azure.com/openai/models\" -H \"api-key: <key>\""
echo "   Expected: 403 Forbidden (public access blocked)"
echo ""
echo "7. Check DNS resolution from VNet (via App Service or VM in VNet):"
echo "   nslookup ${FOUNDRY_NAME}.cognitiveservices.azure.com"
